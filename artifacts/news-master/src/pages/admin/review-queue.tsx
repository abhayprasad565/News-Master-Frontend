...
import { 
  ListChecks, CheckCircle2, XCircle, RotateCcw, AlertTriangle, FileText
} from 'lucide-react';
...
export default function AdminReviewQueue() {
  const { data, isLoading, error } = useGetReviewQueue({ limit: 20 });
  const [selectedJob, setSelectedJob] = useState<any>(null);
  const [actionReason, setActionReason] = useState('');
  const [actionType, setActionType] = useState<'APPROVE' | 'REJECT' | 'REQUEUE' | 'REQUEST_CORRECTION' | null>(null);
  ...
  function handleError(err: any) {
    if (err.statusCode === 409) {
      toast({ 
        ...
        description: 'This item was updated by someone else. Please refresh and try again.',
        variant: 'destructive'
      });
      queryClient.invalidateQueries({ queryKey: getGetReviewQueueQueryKey() });
    } else {
  ...
  function submitAction() {
    ...
    else if (actionType === 'REQUEST_CORRECTION') requestCorrectionMutation.mutate(payload);
  }
      ...
      {isLoading ? (
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-24">
            ...
            <p className="text-sm text-muted-foreground">All caught up! Excellent work.</p>
          </CardContent>
        </Card>
      ) : (
                ...
                <div className="bg-muted/20 p-6 border-t md:border-t-0 md:border-l flex flex-col justify-center gap-3 md:w-64">
                  ...
                  <Dialog>
                    <DialogTrigger asChild>
                      <Button variant="default" className="w-full bg-emerald-600 hover:bg-emerald-700" onClick={() => { setSelectedJob(job); setActionType('APPROVE'); setActionReason(''); }}>
                  ...
                  <Dialog>
                    <DialogTrigger asChild>
                      <Button variant="destructive" className="w-full" onClick={() => { setSelectedJob(job); setActionType('REJECT'); setActionReason(''); }}>
                  ...
                  <Dialog>
                    <DialogTrigger asChild>
                      <Button variant="outline" className="w-full" onClick={() => { setSelectedJob(job); setActionType('REQUEST_CORRECTION'); setActionReason(''); }}>
                  ...
                  <Dialog>
                    <DialogTrigger asChild>
                      <Button variant="secondary" className="w-full" onClick={() => { setSelectedJob(job); setActionType('REQUEUE'); setActionReason(''); }}>
      ...
      <Dialog open={!!actionType} onOpenChange={(open) => !open && setActionType(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Please provide a reason for this action. This will be recorded in the audit log.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setActionType(null)} disabled={isPending}>Cancel</Button>
            <Button 
              onClick={submitAction} 
              disabled={isPending || !actionReason.trim()}
              className={actionType === 'APPROVE' ? 'bg-emerald-600 hover:bg-emerald-700' : actionType === 'REJECT' ? 'bg-destructive hover:bg-destructive/90 text-destructive-foreground' : ''}
            >
