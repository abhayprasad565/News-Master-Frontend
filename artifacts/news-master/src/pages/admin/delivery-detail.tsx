...
import { ArrowLeft, Send, AlertCircle, CheckCircle2, RotateCcw, ShieldAlert, Code } from 'lucide-react';
...
export default function AdminDeliveryDetail() {
  const { id } = useParams();
  const { data: detail, isLoading, error } = useGetDelivery(id || '', {
  ...
  const [reconcileOutcome, setReconcileOutcome] = useState<ReconcileInputOutcome | ''>('');
  ...
  const handleRetry = () => {
    retryMutation.mutate({ deliveryId: delivery.id, data: { reason: retryReason || 'Manual retry' } });
  };
  ...
  const handleReconcile = () => {
    ...
    reconcileMutation.mutate({ 
      ...
      data: { outcome: reconcileOutcome as ReconcileInputOutcome, reason: reconcileReason || 'Manual reconciliation' } 
    });
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <div className="flex flex-wrap items-center gap-2 mt-2">
            ...
            <Badge variant="outline" className="capitalize">{delivery.platform}</Badge>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
            ...
            <Dialog open={retryOpen} onOpenChange={setRetryOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>Trigger another attempt to send this payload.</DialogDescription>
                </DialogHeader>
                <div className="py-4">
                  <Textarea 
                ...
                <DialogFooter>
                  ...
                  <Button onClick={handleRetry} disabled={retryMutation.isPending}>Confirm Retry</Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
          )}
          ...
          {showReconcile && (
            <Dialog open={reconcileOpen} onOpenChange={setReconcileOpen}>
              <DialogTrigger asChild>
                <Button variant="outline" className="border-purple-500 text-purple-600 hover:bg-purple-50 dark:hover:bg-purple-950/30">
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>Manually resolve this delivery if the remote system status is unknown.</DialogDescription>
                </DialogHeader>
                <div className="py-4 space-y-4">
                  <Select value={reconcileOutcome} onValueChange={(val: any) => setReconcileOutcome(val)}>
                    <SelectTrigger>
                ...
                <DialogFooter>
                  ...
                  <Button onClick={handleReconcile} disabled={reconcileMutation.isPending || !reconcileOutcome}>
                    Reconcile
                  </Button>
                </DialogFooter>
              </DialogContent>
      ...
      <div className="grid gap-6 md:grid-cols-2">
          ...
          <CardContent className="space-y-4 text-sm">
            ...
            {delivery.sentAt && (
              <div className="flex justify-between border-b pb-2">
                ...
                <span className="font-medium text-emerald-600">{format(new Date(delivery.sentAt), 'MMM d, HH:mm:ss')}</span>
              </div>
            )}
            {delivery.lastError && (
        ...
        <Card className="col-span-1">
          ...
          <CardContent className="space-y-4 text-sm">
            {publication && (
              <div className="flex flex-col border-b pb-2">
                <span className="text-muted-foreground text-xs mb-1">Publication</span>
              ...
              {!attempts?.length ? (
                ...
                attempts.map((attempt) => (
                  <tr key={attempt.id} className="hover:bg-muted/30">
                    <td className="px-4 py-3 text-center font-medium">{attempt.attemptNumber}</td>
                    <td className="px-4 py-3 text-muted-foreground whitespace-nowrap">
                      {format(new Date(attempt.createdAt), 'MMM d, HH:mm:ss')}
                    </td>
                    <td className="px-4 py-3 font-mono text-xs">{attempt.outcome || '-'}</td>
                    <td className="px-4 py-3 text-center">{attempt.statusCode || '-'}</td>
