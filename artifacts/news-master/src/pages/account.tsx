    ...
    return (
      <div className="mx-auto max-w-md py-16 text-center">
        ...
        <p className="mt-2 text-muted-foreground">
          Create or open a reader account to view account settings.
        </p>
    ...
    <div className="space-y-6 max-w-2xl mx-auto py-8">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-col space-y-2 items-start">
            ...
            <Badge
              variant={user.role === "reader" ? "secondary" : "default"}
              className="flex items-center gap-1"
            >
              {user.role !== "reader" && <ShieldCheck className="h-3 w-3" />}
              {roleLabel(user.role)}
            </Badge>
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Each device has an independently revocable session.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          {sessions.isLoading && <Loader2 className="h-5 w-5 animate-spin" />}
            ...
            >
              ...
              {!session.current && (
                <Button
                  ...
                  variant="outline"
                  onClick={() =>
                    apiFetch(`/api/account/sessions/${session.id}`, {
                      method: "DELETE",
