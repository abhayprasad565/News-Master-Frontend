...
News Master needs a browser frontend with two authenticated experiences:

1. Reader experience: authenticated readers browse published, reader-safe stories.
2. Admin experience: authenticated editors manage review, posts, labels, publications, deliveries, and Instagram/X output records.

The backend now supports HTTP-only cookie sessions for browser login, reader story projections, label management, admin post workflows, publication/delivery inspection, delivery retry/reconciliation, and Instagram/X platform-post record inspection. Existing bearer-token admin auth remains supported for operational/API clients.
...
- Log in and log out with email/password.
...
- Session role embedded as `admin` or `reader`
...
- Use `credentials: "include"` on all browser API calls that need session auth.
...
- Implement app shell, routing, auth screens, API client, and protected routes.
- Integrate `POST /auth/login`, `POST /auth/logout`, and `GET /auth/me`.
